...
const rooms = new Map();
const DEFAULT_ROUNDS = Array.from({length:10}, () => ({ theme: "random", duration: 80 }));
const MAX_HINTS = 2;
...
const OBJECT_WORDS = [
  "cat","dog","dragon","pizza","rocket","wizard","ghost","banana","robot","penguin","castle","volcano","spaceship","unicorn","pirate","ninja","tornado","mermaid","dinosaur","treasure","coffee","headphones","controller","sunglasses","hamburger","ice cream","trophy","guitar","camera","crown","football","basketball","sword","shield","monster","alien","superhero","forest","island","campfire","rainbow","sunset","snowman","octopus","butterfly","shark","elephant","monkey","parrot","koala","school","library","bedroom","birthday","party","roller coaster","train","airplane","bicycle","motorcycle","Discord logo","internet meme","angry potato","sleepy cat","flying toaster","tiny house","giant donut","magic wand"
];
...
io.on("connection",socket=>{
  socket.on("room:create",({name},cb)=>{
    ...
    const code=makeCode();
    const room={code,hostId:socket.id,status:"lobby",roundIndex:-1,rounds:DEFAULT_ROUNDS.map(r=>({...r})),activeRound:null,endsAt:null,drawerId:null,chosenWord:"",users:new Map(),drawings:{},votes:{best:{},worst:{},funniest:{}},chat:[],usedAvatars:new Set(),bannedNames:new Set(),challenges:new Map(),awardTotals:{best:{},worst:{},funniest:{}},playerOptions:new Map(),guessedIds:new Set(),guessOrder:[],hintLevel:0,drawActions:[]};
