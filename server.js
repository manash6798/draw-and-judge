...
const io = new Server(server, { maxHttpBufferSize: 3e6, perMessageDeflate: false, connectionStateRecovery: { maxDisconnectionDuration: 2 * 60 * 1000, skipMiddlewares: true }, pingInterval: 25000, pingTimeout: 20000 });
...
function buildRoundDeck(){
  // Every round is genuinely random. The category is chosen again when the
  // round starts, so a new game never follows a fixed special-round pattern.
  // We only avoid the exact same category twice in a row to keep the game varied.
  return Array.from({length:10},()=>({theme:"random",duration:80}));
}
...
io.on("connection",socket=>{
  ...
  socket.on("word:choose",({word},cb)=>{
    ...
    if(!room||room.status!=="drawing"||room.drawerId!==socket.id)return cb?.({ok:false,error:"You cannot choose a word right now."});
    if(room.chosenWord)return cb?.({ok:false,error:"Your word is already locked."});
    const options=room.playerOptions.get(socket.id)||[];
    word=String(word||"").trim();
    if(!options.includes(word))return cb?.({ok:false,error:"That word is not available."});
    room.chosenWord=word;
    room.hintLevel=0;
    room.startedAt=Date.now();
    room.endsAt=room.startedAt+(room.roundDuration||80)*1000;
    io.to(socket.id).emit("word:chosen",{word,startedAt:room.startedAt,endsAt:room.endsAt});
    io.to(room.code).emit("word:locked",{drawerId:socket.id,hint:maskWord(word,0),hintsUsed:0,maxHints:MAX_HINTS,round:room.activeRound,endsAt:room.endsAt});
    emitRoom(room);
    cb?.({ok:true});
  });
  ...
  socket.on("draw:replace",payload=>{
    const room=rooms.get(socket.data.room); if(!room||room.status!=="drawing"||room.drawerId!==socket.id||!room.chosenWord)return;
    const incoming=Array.isArray(payload?.actions)?payload.actions.slice(-12000):[];
    const clean=[];
    for(const a of incoming){
      if(a?.type==="clear"){clean.push({type:"clear"});continue;}
      if(a?.type==="fill"){const n=v=>Math.max(0,Math.min(1,Number(v)||0));clean.push({type:"fill",x:n(a.x),y:n(a.y),color:String(a.color||"#111111").slice(0,20)});continue;}
      if(a?.type==="stroke"){const st=normalizeStroke(a);if(st)clean.push({type:"stroke",...st});}
    }
  ...
  socket.on("chat",({text})=>{
    ...
    if(room.status==='drawing' && room.drawerId===socket.id) return;
    if(room.status==='drawing'&&room.drawerId&&socket.id!==room.drawerId&&room.chosenWord&&!room.guessedIds.has(socket.id)){
      if(normalizeGuess(text)===normalizeGuess(room.chosenWord)){
        const duration=Math.max(30,room.roundDuration||80);
        const remaining=Math.max(0,Math.ceil((room.endsAt-Date.now())/1000));
        const elapsedRatio=Math.max(0,Math.min(1,remaining/duration));
        const place=room.guessOrder.length;
        // Skribbl-style: faster guesses and earlier correct guessers earn more.
        const speedPoints=Math.round(300*elapsedRatio);
        const orderMultiplier=Math.max(0.45,1-(place*0.10));
        const points=Math.max(50,Math.round(speedPoints*orderMultiplier));
        user.score+=points; user.correct+=1; room.guessedIds.add(socket.id); room.guessOrder.push(socket.id);
        // The artist is rewarded for every successful guess, encouraging clear drawings.
        const artist=room.users.get(room.drawerId);
        const drawerBonus=Math.max(25,Math.round(points*0.35));
        if(artist) artist.score+=drawerBonus;
        io.to(room.code).emit("guess:correct",{userId:user.id,name:user.name,avatar:user.avatar,points,total:user.score,place:place+1,drawerBonus,drawerId:room.drawerId});
        emitRoom(room);
        const eligible=connectedUsers(room).filter(u=>u.id!==room.drawerId).length;
        if(eligible>0 && room.guessedIds.size>=eligible) enterVoting(room);
        return;
      }
    }
...
setInterval(()=>{
  for(const room of rooms.values()){
    if(room.status!=="drawing"||!room.chosenWord||!room.endsAt) continue;
    const remaining=Math.max(0,Math.ceil((room.endsAt-Date.now())/1000));
    if(room.chosenWord){
