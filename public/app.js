...
function queueStroke(a,b){pendingStrokes.push({tool,color,size:+$('size').value,fill:$('fillShape').checked,x1:a.x,y1:a.y,x2:b.x,y2:b.y});if(!strokeFlushTimer)strokeFlushTimer=setTimeout(flushStrokes,40)}
...
function emitStroke(a,b){queueStroke(a,b)}
...
canvas.addEventListener('pointerdown',e=>{if(!isDrawer())return;const p=pointerPos(e);if(tool==='fill'){if(fillCanvasAt(p,color)){socket.emit('draw:fill',{x:p.x,y:p.y,color});sound('pop')}return}drawing=true;canvas.setPointerCapture(e.pointerId);shapeStart=p;if(['line','rect','circle','triangle','diamond','star','arrow'].includes(tool)){shapeBase=ctx.getImageData(0,0,canvas.width,canvas.height)}else{drawSegment(shapeStart,shapeStart,{tool,color,size:+$('size').value});emitStroke(shapeStart,shapeStart)}});
canvas.addEventListener('pointermove',e=>{if(!drawing||!isDrawer())return;const p=pointerPos(e);if(['line','rect','circle','triangle','diamond','star','arrow'].includes(tool)){scheduleShapePreview(p)}else{drawSegment(shapeStart,p,{tool,color,size:+$('size').value});emitStroke(shapeStart,p);shapeStart=p}});
canvas.addEventListener('pointerup',e=>{if(!drawing)return;if(['line','rect','circle','triangle','diamond','star','arrow'].includes(tool)){const p=pointerPos(e);restoreShapePreview(p);emitStroke(shapeStart,p)}drawing=false;shapeStart=null;shapeBase=null;previewPoint=null;if(previewRAF){cancelAnimationFrame(previewRAF);previewRAF=0}flushStrokes()});
canvas.addEventListener('pointercancel',()=>{drawing=false;shapeStart=null;shapeBase=null;previewPoint=null;if(previewRAF){cancelAnimationFrame(previewRAF);previewRAF=0}});
...
socket.on('draw:clear',()=>{if(room?.status==='drawing'&&!isDrawer())clearCanvas(false)});
...
socket.on('round:options',m=>{if(!isDrawer())return;const box=$('wordChoices');box.innerHTML=`<div class="choice-head"><span>${esc(m.type?.title||'Choose your word')}</span><small>${esc(m.type?.subtitle||'Pick one')}</small></div><div class="choice-grid">${(m.options||[]).map(w=>`<button class="word-choice" data-word="${esc(w)}">${esc(w)}</button>`).join('')}</div>`;box.classList.remove('hidden');$('wordHint').textContent='Your choice is secret. Pick one and start drawing.';box.querySelectorAll('.word-choice').forEach(b=>b.onclick=()=>socket.emit('word:choose',{word:b.dataset.word}))});
socket.on('word:chosen',m=>{if(!isDrawer())return;$('prompt').textContent=`Draw: ${m.word}`;$('wordHint').textContent='Word locked — the room is guessing';document.querySelectorAll('.word-choice').forEach(b=>{b.classList.toggle('selected',b.dataset.word===m.word);b.disabled=true})});
socket.on('word:locked',m=>{if(m.drawerId===socket.id)return;$('prompt').textContent='Guess the drawing';$('wordHint').textContent=`Hint 0/2: ${m.hint||'_____'}  •  fastest correct guesses score the most`});
...
socket.on('guess:correct',m=>{sound('correct');confetti(16);systemMessage(`🎯 ${m.avatar} ${m.name} guessed it! +${m.points} points`, 'correct-guess');toast(`${m.name} +${m.points} points`);updatePlayers()});
